/**
 * @file dme7_simulator.c
 * @brief Serveur OSC (substitut du processeur Yamaha DME7)
 * @author Jonathan Ntoula
 * @date Mai 2026
 * * Ce module simule la réception de messages de spatialisation et de contrôle
 * provenant d'un client OSC, en utilisant les outils de la bilbliothèque liblo.
 */
...
/**
 * @brief Traitement des messages OSC entrants.
 * 
 * Cette fonction est appelée à chaque réception de message OSC
 * Elle intègre un mécanisme de contrôle temporel 
 * pour limiter l'affichage en console en cas de flood.
 * 
 * @param path L'adresse OSC reçue (ex: /yosc:req/...).
 * @param types La chaîne de caractères décrivant les types d'arguments (ex: "fff").
 * @param argv Tableau de pointeurs vers les valeurs des arguments.
 * @param argc Nombre d'arguments reçus.
 * @param data Message OSC brut (non utilisé ici).
 * @param user_data Pointeur vers des données utilisateur (non utilisé ici).
 * @return int Retourne toujours 0 pour confirmer le traitement du message à liblo.
 */

// Gestionnaire de message yOSC DME7  
...
/**
 * @brief Callback en cas d'erreur liblo.
 * 
 * @param num Code d'erreur.
 * @param m Message d'erreur textuel.
 * @param path Chemin ou adresse ayant provoqué l'erreur.
 */
...
/**
 * @brief Point d'entrée principal du simulateur.
 * 
...
int main() {
    ...
    // gestion des appels système socket() / bind() et création d'un thread 
